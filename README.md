...
> 실제 환경에서 촬영된 영상은 센서의 물리적 한계로 노이즈가 포함되기 때문에, 노이즈를 제거한 후에야 실질적인 응용이 가능하다. 노이즈 제거를 위해 최근에는 인공지능 기반 영상 복원 네트워크가 활발히 개발되고 있으며, 특히 합성곱 및 트랜스포머 기반 접근 방식을 결합하여 높은 성능을 달성하는 네트워크가 주목받고 있다. 이에 본 논문에서는 트랜스포머의 어텐션 연산에서 채널 및 합성곱 어텐션을 효과적으로 결합한 네트워크를 제안하였다. 
