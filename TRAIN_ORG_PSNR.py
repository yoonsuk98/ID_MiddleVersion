    ...
    for epoch in range(1000000):  # keep running
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_save'] == 0 and opt['rank'] == 0:
                logger.info('---Period Saving the model.---')
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 and opt['rank'] == 0:
                ...
                for test_data in tqdm(test_loader):
                    ...
                    elif opt['netG']['net_type']=='chair':
                        visuals = model.current_visuals_chair()
                        E_img = util.tensor2uint(visuals['E'])
                        H_img = util.tensor2uint(visuals['H'])

                    elif opt['netG']['net_type']=='rdsr33_2':
                        visuals = model.current_visuals_rdsr33_2()
                        E_img = util.tensor2uint(visuals['E'])
                        H_img = util.tensor2uint(visuals['H'])
                                        
                    elif opt['netG']['net_type']=='lowlight_zerodce' or opt['netG']['net_type']=='lowlight_safmdce' or opt['netG']['net_type']=='lowlight_zerodce_denoising' : # or opt['netG']['net_type']=='lowlight_zerodce_denoisingours':
                        visuals = model.current_visuals_lowlight()
                        E_img = util.tensor2uint(visuals['E'])
                        H_img = util.tensor2uint(visuals['H'])
