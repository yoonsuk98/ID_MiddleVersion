...
class Quaternion_concat(torch.nn.Module):
...
class LayerNorm_Quaternion(nn.Module):
    def __init__(self, features, eps=1e-6):
        super(LayerNorm_Quaternion, self).__init__()
...
def combine_last_two_dimensions(x):
    ...
    x_shape = list(x.size())
    a, b = x_shape[-2:]
    return x.reshape(x_shape[:-2] + [a * b])
...
def combine_heads(x):
      ...
      output = []
      for _x in x:
          _x=combine_last_two_dimensions(_x.permute(0, 2, 1, 3))
          output.append(_x)
...
def split_heads(x,num_heads):
    ...
    output=[]
    for _x in x:
        _x= split_last_dimension(_x,num_heads)
...
def quarternion_concat(x,dim=2):
    '''
    x=[x1,x2,..]
    dim=2 in case of x1: bsz x al x dim
    dim=1 in case of x1: bsz x channel x height x weight
    '''
...
def quaternion_attention(a, b):
    ...
    #r = ar @ br.transpose(-2, -1) - ai @ bi.transpose(-2, -1) - aj @ bj.transpose(-2, -1) - ak @ bk.transpose(-2, -1)
    #i = ar @ bi.transpose(-2, -1) + ai @ br.transpose(-2, -1) + aj @ bk.transpose(-2, -1) - ak @ bj.transpose(-2, -1)
    #j = ar @ bj.transpose(-2, -1) - ai @ bk.transpose(-2, -1) + aj @ br.transpose(-2, -1) + ak @ bi.transpose(-2, -1)
    #k = ar @ bk.transpose(-2, -1) + ai @ bj.transpose(-2, -1) - aj @ bi.transpose(-2, -1) + ak @ br.transpose(-2, -1)
    #r = ar @ br.transpose(-2, -1)
    #i = ai @ bi.transpose(-2, -1)
    #j = aj @ bj.transpose(-2, -1)
    #k = ak @ bk.transpose(-2, -1)
    att = a@b.transpose(-2, -1)
    return [att, att, att, att]


def quaternion_dot_product_attention(q,k,v,bias=0.):
    v_vals = torch.chunk(v, 4, dim=-1)
    output=[]
    all_logits = quaternion_attention(q, k)

    for i, logits in enumerate(all_logits):

        weight=torch.nn.Softmax(dim=-1)(logits)+bias
        print(weight.shape)

        o = torch.matmul(weight,v_vals[i])
        output.append(o)

    return torch.cat((output[0],output[1],output[2],output[3]),dim=-1)
...
def quaternion_transpose_conv_rotation(input, zero_kernel, r_weight, i_weight, j_weight, k_weight, bias, stride,
    ...
    return convfunc(input, cat_kernels_4_quarternion, bias, stride, padding, output_padding, groups, dilatation)
