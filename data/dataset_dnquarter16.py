...
class DatasetDnCNN(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            # add noise
            # --------------------------------
            noise = torch.randn(img_L.size()).mul_(self.sigma/65535.0)
            img_L.add_(noise)

            img_H = util.tensor32quaternion(img_H)
            img_L = util.tensor32quaternion(img_L)
            ...
            img_L += np.random.normal(0, self.sigma_test/65535.0, img_L.shape)

            # --------------------------------
            # HWC to CHW, numpy to tensor
            # --------------------------------
            img_L = util.single2quaternion(img_L)
