...
def get_ima_paths(dataroot):
    paths = None  # return None if dataroot is None
    if isinstance(dataroot, str):
        paths = sorted(_get_paths_from_npy(dataroot))
    elif isinstance(dataroot, list):
        paths = []
        for i in dataroot:
            paths += sorted(_get_paths_from_npy(i))
    return paths

def _get_paths_from_npy(path):
    ...
    for dirpath, _, fnames in natsorted(os.walk(path)):
        for fname in natsorted(fnames):
            ...
            npy.append(img_path)
    assert npy, '{:s} has no valid image file'.format(path)
    return npy
...
def denormalize_(img):
    img = (img * 4096.0) - 1024.0 # (self.norm_range_max - self.norm_range_min) + self.norm_range_min
    return img
    
def trunc(img: torch.Tensor, vmin: float = -160.0, vmax: float = 240.0) -> torch.Tensor:
    return torch.clamp(img, min=vmin, max=vmax)
...
# --------------------------------------------
# quaternion tensor 4xHxW --->  images HxWxC
# --------------------------------------------
def quatertensor2uint(img):
    img = img[1:, :, :]
    img = img.data.squeeze().float().clamp_(0, 1).cpu().numpy()
    if img.ndim == 3:
        img = np.transpose(img, (1, 2, 0))
...
def quatertensor2uint16_1ch(img):
    img = img[3:4, :, :]
    img = img.data.squeeze().float().clamp_(0, 1).cpu().numpy()
    if img.ndim == 3:
        img = np.transpose(img, (1, 2, 0))
...
def quatertensor2uint_move(img):
    img = torch.subtract(img[1:, :, :], img[:, 0, :, :])
    img = img.data.squeeze().float().clamp_(0, 1).cpu().numpy()
    if img.ndim == 3:
        img = np.transpose(img, (1, 2, 0))
    return np.uint8((img * 255.0).round())
...
# quaternion tensor
def tensor32quaternion(img):
    _, W, H = img.shape
    pad = torch.zeros(1, W, H)
    return torch.cat([pad, img], dim=0)

def tensor32quaternion_1ch(img):
    _, W, H = img.shape
    pad = torch.zeros(3, W, H)
    return torch.cat([pad, img], dim=0)


# convert single (HxWx3) to 4xHxW quaternion tensor
def single2quaternion(img):
    npad = ((0, 0), (0, 0), (1, 0))
    img = np.pad(img, pad_width=npad, mode='constant', constant_values=0)
    return torch.from_numpy(np.ascontiguousarray(img)).permute(2, 0, 1).float()

def single2quaternion_1ch(img):
    npad = ((0, 0), (0, 0), (3, 0))
    img = np.pad(img, pad_width=npad, mode='constant', constant_values=0)
    return torch.from_numpy(np.ascontiguousarray(img)).permute(2, 0, 1).float()
#############################################################################
...
def calculate_rmse_medical(img1, img2):
    if type(img1) == torch.Tensor:
        return torch.sqrt(torch.mean((img1 - img2)**2)).item()
    else:
        return np.sqrt(np.mean((img1 - img2)**2))

def calculate_psnr_medical(img1, img2, data_range):
...
def calculate_ssim_medical(img1, img2, data_range):
    '''calculate SSIM
    ...
    if img1.ndim == 2:
        return ssim_medical(img1, img2, data_range)
    elif img1.ndim == 3:
        if img1.shape[2] == 3:
            ssims = []
            for i in range(3):
                ssims.append(ssim_medical(img1[:,:,i], img2[:,:,i], data_range))
            return np.array(ssims).mean()
        elif img1.shape[2] == 1:
            return ssim_medical(np.squeeze(img1), np.squeeze(img2), data_range)
    else:
        raise ValueError('Wrong input image dimensions.')
    
def ssim_medical(img1, img2, data_range):
